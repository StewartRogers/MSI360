...
export const pt: Translation = {
  ...
  questions: {
    ...
    "question-10": {
      label:
        "Utilizando a tabela abaixo, indique as partes específicas do corpo onde sentiu dor ou desconforto relacionado com o trabalho durante ou após a execução do trabalho ou tarefa que está a ser avaliado.\n\nIndique se a) um ou ambos os lados do corpo estiveram envolvidos, e b) a dor durou 2 ou mais dias.",
      groups: {
        neck: { label: "1. Pescoço", options: bodySideOptions },
        shoulders_upper_arms: { label: "2. Ombro(s) ou braço(s) superior(es)", options: bodySideOptions },
        elbows_forearms: { label: "3. Cotovelo(s) ou antebraço(s)", options: bodySideOptions },
        wrists_hands_fingers: { label: "4. Pulso(s), mão(s) ou dedos", options: bodySideOptions },
        lower_back: { label: "5. Zona lombar", options: bodySideOptions },
        hips_upper_legs: { label: "6. Anca(s) ou parte superior da(s) perna(s)", options: bodySideOptions },
        knees_lower_legs: { label: "7. Joelho(s) ou parte inferior da(s) perna(s)", options: bodySideOptions },
        ankles_feet: { label: "8. Tornozelo(s) ou pé/pés", options: bodySideOptions }
      }
    },
    "question-11": {
      label:
        "Pense num dia de trabalho típico ou, se estiver a avaliar uma tarefa ou atividade específica, na parte do dia de trabalho em que realiza essa tarefa. Quanto tempo passa inclinado/a ou a apoiar parte do corpo em objetos ou arestas afiadas?",
      options: {
    ...
    "question-13": {
      label:
        "Pense nos tipos de ferramentas ou objetos que segura durante mais de 30 minutos de cada vez. Na lista de descrições abaixo, selecione todas as afirmações que se aplicam a estes itens.\n\nA última opção não pode ser selecionada se qualquer outra opção tiver sido assinalada.",
      options: {
    ...
    "question-14": {
      label:
        "Utiliza alguma parte do corpo como ferramenta improvisada para concluir o seu trabalho? Por exemplo, pode usar a palma da mão ou o joelho para aplicar força numa superfície.\n\nA ilustração abaixo é um exemplo de utilização do corpo desta forma.",
      options: {
    ...
    "question-15": {
      label:
        "Pense nos tipos de superfícies sobre as quais empurra, puxa ou move objetos durante o trabalho, tarefa ou atividade que está a avaliar hoje. Na lista de descrições abaixo, selecione todas as afirmações que se aplicam a essas superfícies.",
      options: {
    ...
    "question-20": {
      label:
        "Pense num dia de trabalho típico ou, se estiver a avaliar uma tarefa ou atividade específica, na parte do dia de trabalho em que realiza essa tarefa. Quando está sentado/a ou de pé, com que frequência trabalha com a parte superior do corpo inclinada para a frente, para trás ou para o lado?\n\nSelecione as opções que se aplicam a si.",
      groups: {
        forward_backward: { label: "Inclino-me para a frente ou para trás mais de 15 graus", options: mostSomeNeverOptions },
        sideways: { label: "Inclino-me para o lado", options: mostSomeNeverOptions }
      }
    },
    "question-21": {
    ...
    "question-22": {
      label:
        "Pense num dia de trabalho típico ou, se estiver a avaliar uma tarefa ou atividade específica, na parte do dia de trabalho em que realiza essa tarefa. Quando está sentado/a ou de pé, indique onde estão as suas mãos em relação ao corpo.\n\nSelecione as opções que se aplicam a si.",
      groups: {
        hands_above_shoulders: { label: "Mãos acima dos ombros", options: mostSomeNeverOptions },
        hands_floor_to_knee: { label: "Mãos entre o chão e o joelho", options: mostSomeNeverOptions }
      }
    },
    "question-23": {
    ...
    "question-29": {
      label:
        "Pense num dia de trabalho típico ou, se estiver a avaliar uma tarefa ou atividade específica, na parte do dia de trabalho em que realiza essa tarefa. Quanto tempo passa a executar movimentos semelhantes repetidamente?",
      options: {
        less_than_30_min: "Menos de 30 minutos por dia",
    ...
    "question-33": {
      label:
        "Quando segura um objeto entre o polegar e as pontas dos dedos, chama-se pega em pinça. Ao realizar o trabalho, tarefa ou atividade que está a avaliar hoje, quanto tempo passa a usar uma pega em pinça para segurar um objeto com mais de 2 libras / 1 kg?",
      options: {
    ...
    "question-37": {
      label:
        "Pense num dia de trabalho típico ou, se estiver a avaliar uma tarefa ou atividade específica, na parte do dia de trabalho em que realiza essa tarefa. Alguma vez se distrai com ruídos, como sirenes, conversas altas, trânsito, etc.?",
      options: {
        frequently: "Frequentemente",
        sometimes: "Às vezes",
        no: "Não"
