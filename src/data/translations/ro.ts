...
export const ro: Translation = {
  app: {
    ...
    description_body:
      "Urmatoarele intrebari sunt despre munca pe care o faci intr-o zi obisnuita de lucru sau atunci cand finalizezi sarcina ori activitatea specifica pe care doresti sa o evaluezi astazi. Scopul este sa ii spui MSI360 ce actiuni faci pentru a-ti realiza munca."
  },
  sections: {
    intro: "Despre munca",
  ...
  questions: {
    ...
    "question-10": {
      label:
        "Folosind tabelul de mai jos, indica partile specifice ale corpului unde ai avut durere sau disconfort legat de munca in timpul sau dupa efectuarea postului ori sarcinii evaluate.\n\nIndica daca a) a fost implicata una sau ambele parti ale corpului si b) durerea a durat 2 zile sau mai mult.",
      groups: {
        neck: { label: "1. Gat", options: bodySideOptions },
        shoulders_upper_arms: { label: "2. Umeri sau brate superioare", options: bodySideOptions },
        elbows_forearms: { label: "3. Coate sau antebrate", options: bodySideOptions },
        wrists_hands_fingers: { label: "4. Incheieturi, maini sau degete", options: bodySideOptions },
        lower_back: { label: "5. Partea inferioara a spatelui", options: bodySideOptions },
        hips_upper_legs: { label: "6. Solduri sau coapse", options: bodySideOptions },
        knees_lower_legs: { label: "7. Genunchi sau gambe", options: bodySideOptions },
        ankles_feet: { label: "8. Glezne sau picioare", options: bodySideOptions }
      }
    },
    "question-11": {
      label:
        "Gandeste-te la o zi obisnuita de lucru sau, daca evaluezi o sarcina specifica, la partea zilei in care o faci. Cat timp petreci sprijinind sau rezemand o parte a corpului pe obiecte ori margini ascutite?",
      options: {
    ...
    "question-13": {
      label:
        "Gandeste-te la tipurile de unelte sau obiecte pe care le tii in mana mai mult de 30 de minute odata. Din lista de mai jos, selecteaza toate afirmatiile care se aplica.\n\nUltima optiune nu poate fi selectata daca a fost bifata orice alta optiune.",
      options: {
    ...
    "question-14": {
      label:
        "Folosesti vreo parte a corpului ca unealta improvizata pentru a-ti finaliza munca? De exemplu, ai putea folosi palma sau genunchiul pentru a aplica forta pe o suprafata.\n\nImaginea de mai jos este un exemplu de folosire a corpului in acest fel.",
      options: {
    ...
    "question-15": {
      label:
        "Gandeste-te la tipurile de suprafete peste care impingi, tragi sau deplasezi obiecte in timpul postului, sarcinii sau activitatii evaluate astazi. Selecteaza toate descrierile care se aplica.",
      options: {
    ...
    "question-20": {
      label:
        "Gandeste-te la o zi obisnuita de lucru sau la partea zilei in care faci sarcina evaluata. Cand stai jos sau in picioare, cat de des lucrezi cu partea superioara a corpului aplecata inainte, inapoi sau lateral?\n\nSelecteaza optiunile care ti se aplica.",
      groups: {
        forward_backward: { label: "Ma aplec inainte sau inapoi mai mult de 15 grade", options: mostSomeNeverOptions },
        sideways: { label: "Ma aplec lateral", options: mostSomeNeverOptions }
      }
    },
    "question-21": {
    ...
    "question-22": {
      label:
        "Gandeste-te la o zi obisnuita de lucru sau la partea zilei in care faci sarcina evaluata. Cand stai jos sau in picioare, indica unde se afla mainile in raport cu corpul.\n\nSelecteaza optiunile care ti se aplica.",
      groups: {
        hands_above_shoulders: { label: "Mainile deasupra umerilor", options: mostSomeNeverOptions },
        hands_floor_to_knee: { label: "Mainile intre podea si genunchi", options: mostSomeNeverOptions }
      }
    },
    "question-23": {
    ...
    "question-29": {
      label:
        "Gandeste-te la o zi obisnuita de lucru sau la partea zilei in care faci sarcina evaluata. Cat timp petreci efectuand miscari similare in mod repetat?",
      options: {
        less_than_30_min: "Mai putin de 30 de minute pe zi",
    ...
    "question-33": {
      label:
        "Cand prinzi un obiect intre degetul mare si varfurile degetelor, se numeste prindere in penseta. In timpul postului, sarcinii sau activitatii evaluate astazi, cat timp petreci folosind o prindere in penseta pentru a tine un obiect mai greu de 2 livre / 1 kg?",
      options: {
    ...
    "question-34": {
      label:
        "Cand iti infasori mana in jurul unui obiect pentru a-l tine, se numeste prindere de forta. Cat timp petreci folosind o prindere de forta pentru a tine un obiect mai greu de 10 livre / 4,5 kg?",
      options: {
        less_than_2_hours: "Mai putin de 2 ore pe zi",
        more_than_2_hours: "Mai mult de 2 ore pe zi",
        none: "Deloc"
    ...
    "question-36": {
      label:
        "Daca impingi sau tragi obiecte mai grele de 80 lb / 36 kg, cat timp petreci mutand aceste obiecte peste suprafete aspre, cum ar fi pietris, gresie sau teren denivelat, ori peste suprafete moi, cum ar fi nisip, noroi sau iarba?",
      options: {
        less_than_5_min: "Mai putin de 5 minute pe zi",
    ...
    "question-37": {
      label:
        "Gandeste-te la o zi obisnuita de lucru sau la partea zilei in care faci sarcina evaluata. Esti vreodata distras de zgomote, cum ar fi sirene, vorbit tare, trafic etc.?",
      options: {
        frequently: "Frecvent",
        sometimes: "Uneori",
        no: "Nu"
