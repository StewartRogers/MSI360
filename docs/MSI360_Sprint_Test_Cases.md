...
| AT-13 | `tests/unit/ai.test.ts` | Unit test | `interpretTextAnswer` uses the local English fallback and returns allowed routing tags when Gemini is not configured. | Fallback returns useful routing tags for heavy lifting/tool use and missing-detail prompts for object weight and duration. |
...
| AT-83 | `tests/unit/ai.test.ts` | Unit test | Expanded English fallback routes common job, task, and combined-detail descriptions to existing tags. | Software, office, warehouse, healthcare, tool-use, cleaning, construction, landscaping, driving, manufacturing, retail, inspection, environmental, load-handling, grip, posture, contact-stress, and work-pace prompts produce expected canonical routing tags. |
| AT-84 | `tests/unit/ai.test.ts` | Unit/routing test | English fallback keeps vague occupations broad, avoids obsolete tags, and routes software developer prompts to office follow-ups. | Vague occupations do not add unsupported high-risk tags, obsolete tag IDs are absent, and a software developer work-from-home prompt makes office follow-up questions visible. |
...
| AT-99 | `tests/unit/send-report.test.ts` | Unit test | `isValidEmail` accepts well-formed addresses and rejects malformed ones. | Well-formed addresses pass; empty strings and strings without an `@`/domain fail. |
| AT-100 | `tests/unit/send-report.test.ts` | Unit test | `isWithinPdfSizeCap` enforces the base64 length ceiling. | Payloads under the cap pass; empty and over-cap payloads fail. |
| AT-101 | `tests/unit/send-report.test.ts` | Unit test | `escapeHtml` neutralizes markup-significant characters before HTML interpolation. | `<`, `>`, `&`, `"`, and `'` are all escaped to their HTML entities. |
| AT-102 | `tests/unit/send-report.test.ts` | Unit test | `isRateLimited` allows requests under the per-IP limit and blocks beyond it. | The first 5 requests from a client within the window return `false`; the 6th returns `true`. |
...
| MT-23 | Email report delivery | API/manual integration test | With `RESEND_API_KEY`/`RESEND_FROM_EMAIL` configured, complete the flow and enter the Resend account's own email address (required while using the shared testing domain) on the Email screen, then click Continue. | The Continue button shows the busy/Processing state briefly, the app reaches the report-ready screen, and the report email arrives (inbox or spam) with the PDF attached matching the on-screen summary. |
| MT-24 | Email report failure fallback | API/manual integration test | Temporarily unset or break `RESEND_API_KEY` (or block the request), then repeat the Email screen flow with an address entered. | A dismissible toast reports the failed send, the app still reaches the report-ready screen, and Download PDF still works. |
